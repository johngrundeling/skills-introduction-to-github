...
<title>BioKissed SA — Shop</title>
<script src="https://js.paystack.co/v2/inline.js"></script><style>
...
async function pay(){
 ...
 const reset=()=>{btn.disabled=false;btn.textContent="Pay with Paystack";};
 try{
  ...
  if(!r.ok||!(data.access_code||data.authorization_url)){ throw new Error(data.error||"Could not start payment."); }
  // Preferred: on-site popup (card, EFT, Capitec Pay, Apple Pay on supported devices).
  if(window.PaystackPop && data.access_code){
   const popup=new PaystackPop();
   popup.resumeTransaction(data.access_code,{
    onSuccess:(t)=>{ localStorage.removeItem("bk_cart"); location.href="/success.html?reference="+encodeURIComponent(t.reference||data.reference); },
    onCancel:()=>{ reset(); },
    onError:(err)=>{ reset(); showErr((err&&err.message)||"Payment could not be completed."); },
   });
   return;
  }
  // Fallback: full-page hosted checkout (also shows all channels).
  window.location.href=data.authorization_url;
 }catch(e){ reset(); showErr(e.message); }
}
